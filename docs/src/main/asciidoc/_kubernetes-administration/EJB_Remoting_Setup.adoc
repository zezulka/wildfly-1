[[EJB_Remoting_Setup]]
= EJB Remoting Setup on Kubernetes

This chapter explains how to setup EJB Remoting to work property on Kubernetes.

link:Developer_Guide{outfilesuffix}#EJB_invocations_from_a_remote_server_instance[this chapter]

[[ejb-setup]]
== Deployment of EJBs on Kubernetes

The WildFly deployed on Kuberentes will most probably utilize the features
which the Kubernetes provide. The server will be 

If a user deploys an EAP instance at the OpenShift he considers ease use of scaling features that OpenShift offers.
For EAP to work correctly with EJB remoting calls between different EAP clusters on OpenShift and for transaction recovery working safely few things need to be done.
