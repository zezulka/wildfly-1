...
public final class TxTestUtil {
    // default wait time is set to 2.5 seconds
    ...
    public static void waitForTimeout(TransactionManager tm) throws SystemException, InterruptedException {
        waitForTimeout(tm, timeoutWaitTime_ms);
    }

    public static void waitForTimeout(TransactionManager tm, long maxWaitTimeMs) throws SystemException, InterruptedException {
        // waiting for timeout
        long startTime = System.currentTimeMillis();
        while(System.currentTimeMillis() - startTime < maxWaitTimeMs && tm.getStatus() == Status.STATUS_ACTIVE) {
            Thread.sleep(200);
        }
        if(tm.getStatus() == Status.STATUS_ACTIVE) {
            throw new IllegalStateException("waitForTimeout exeeded timeout wait time defined as " + maxWaitTimeMs + " ms"
                + " and transaction is still active (was not timeouted)");
        }
    }
    ...
    public static void checkTransactionExists(TransactionManager tm, boolean isExpectTransaction) {
        try {
            ...
            if(!isExpectTransaction && tx != null && tx.getStatus() != Status.STATUS_NO_TRANSACTION) {
                throw new RuntimeException("We do not expect transaction would be active - we haven't activated it in BMT bean");
            } else if (isExpectTransaction && (tx == null || tx.getStatus() != Status.STATUS_ACTIVE)) {
                throw new RuntimeException("We do expect tranaction would be active - we have alredy activated it in BMT bean");
            }
