...
@RunAsClient
public class StatefulLoadbalancingTestCase {
    private static final Logger log = Logger.getLogger(StatefulLoadbalancingTestCase.class);
    ...
    private void validateBalancing(Map<String, Integer> nodeNameNumberOfCalls) {
        ...
        for(String nodeName: nodeNameNumberOfCalls.keySet()) {
            ...
            Assert.assertTrue("Node " + nodeName + " has " + currentPercentageInString + " created beans from all " + totalCalls + 
                    " but it does not match with supposed percentage " + supposedPercentageInString + "+-" + PERCENTAGE_TOLERANCE,
                    (currentPercentage >= supposedPercentage - PERCENTAGE_TOLERANCE) && (currentPercentage <= supposedPercentage + PERCENTAGE_TOLERANCE));
    ...
    public void stopAndClean(
            ...
            @OperateOnDeployment(DEPLOYMENT_4) @ArquillianResource ManagementClient client4) throws Exception {
        log.info("Stop&Clean...");
